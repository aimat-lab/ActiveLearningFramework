...
from example_implementation.al_implementations.oracle import MethanolOracle
from example_implementation.al_implementations.pl import MethanolPL
from example_implementation.al_implementations.stream import MethanolStream
...
class MethanolInitiator(InitiationHelper):

    def __init__(self, x, x_test, y, y_test, internal_lock: Lock, entity=properties.eval_entities["ia"]):
        self._scenario = Scenarios.SbS
        self._entity = entity

        host, user, password, database = "localhost", "root", "roottoor", properties.RUN_NUMBER + "__methanol__" + entity
        ...
        self._pl: PassiveLearner = MethanolPL(x_test=x_test, y_test=y_test, entity=entity, lock=internal_lock)
        self._ro_pl: ReadOnlyPassiveLearner = MethanolPL(x_test=x_test, y_test=y_test, entity=entity, lock=internal_lock)

        lowest = int(np.argsort(map_flat_output_to_shape(y)[0])[0])
        self._stream = MethanolStream(x[lowest], y[lowest], self._ro_pl)
